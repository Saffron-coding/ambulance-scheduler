...
class Job(db.Model):
    ...
    incident_name:so.Mapped[str] = so.mapped_column(db.String, unique=True)
    user_assigned_category: so.Mapped[str] = so.mapped_column(db.Column(db.String(30), nullable=False))
    system_assigned_category: so.Mapped[str] = so.mapped_column(db.Column(db.String(30), nullable=False))
    call_time: so.Mapped[datetime] = so.mapped_column(db.DateTime, nullable=False)
    location: so.Mapped[str] = so.mapped_column(db.Column(db.String(30), nullable=False))
    ...
    patient_notes: so.Mapped[str] = so.mapped_column(db.Column(db.String(500), nullable=False))
    medical_history: so.Mapped[str]= db.Column(db.String(500), nullable=False)
    time: so.Mapped[datetime] = so.mapped_column(db.DateTime, nullable=False)
...
class Ambulance(db.Model):
    ...
    location: so.Mapped[str] = so.mapped_column(db.Column(db.String(150), nullable=False))
    lat: so.Mapped[float] = so.mapped_column(db.Float, nullable=False)
    lon: so.Mapped[float] = so.mapped_column(db.Float, nullable=False)
